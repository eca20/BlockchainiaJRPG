...
namespace FPSRetroKit
{
	...
	{
		//WEAPON: Shotgun TEMPLATE

		[Header("Prefabs and Sprites")]
		public GameObject bloodSplat; //GameObject of blood after projectile hits enemy
		public Sprite idleShotgun; //Shotgun sprite when idle
		public Sprite shotShotgun; //Pistol sprite when shooting

		[Header("Pistol Settings")]
		public float shotgunDamage; //How much damage pistol deals
		public float shotgunRange; //How far can we shoot with Pistol
		...
		public GameObject bulletHoleShotgun; //Game Object for holes that projectiles make once they hit environment
		...
		void FixedUpdate()
		{
			...
			Vector2 bulletOffset = Random.insideUnitCircle * (DynamicCrosshair.spread + 10);
			...
			if (isShot == true && ammoClipLeft > 0 && isReloading == false)
			{
				...
				ammoClipLeft = ammoClipLeft - 2; //Remove 2 ammo from shotgun
				...
				if (Physics.Raycast(ray, out hit, shotgunRange))
				{
					...
					else
					{
						...
						bulletHoleShotgun.transform.GetChild(0).gameObject.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
						...
						//We have two bullet holes as we shot twice in one shotgun reload
						Instantiate(bulletHoleShotgun, hit.point, Quaternion.FromToRotation(Vector3.up, hit.normal)).transform.parent =
		...
		IEnumerator shot()
		{
			GetComponent<SpriteRenderer>().sprite = shotShotgun; //Show Sprite of Shooting (Change graphic on screen)
			yield return new WaitForSeconds(0.5f); //Wait 0.5second
			GetComponent<SpriteRenderer>().sprite = idleShotgun; //Change back to idle weapon after we shot
		}
